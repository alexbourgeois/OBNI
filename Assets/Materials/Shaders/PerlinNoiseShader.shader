Shader "Noise/PerlinNoiseShader" {
	Properties{
		...
		//_Gain("Gain", float) = 1
		//_NoiseStyle("NoiseStyle", int) = 0
		//_Octave("_Octave", int) = 1

		////_Gradient4D("_Gradient4D", 2D) = "white" {}
		////_PermTable1D("_PermTable1D", 2D) = "white" {}
		////_PermTable2D("_PermTable2D", 2D) = "white" {}
	}
	SubShader{
		Pass
		{
			...
			{
				float4 p = float4(IN.texcoord.xy,_Time.y*_Speed, _Time.y*_Speed);//0.01f);//_Time.y/500);
				float n = 0;
